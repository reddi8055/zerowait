...
export default function MapPage() {
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col font-sans tracking-tight text-slate-800 relative overflow-hidden pt-[104px]">
      
      {/* ── SUB-HEADER CONTROL SHELL ── */}
      <div className="px-4 mt-4 relative z-20">
        <div className="max-w-7xl mx-auto bg-white border border-slate-200/60 shadow-sm px-6 py-4 rounded-2xl flex flex-col lg:flex-row items-center justify-between gap-4 transition-all duration-300">
          
          {showSearch ? (
            <div className="flex-1 flex flex-col md:flex-row items-center gap-4 w-full animate-in fade-in zoom-in-95 duration-200">
              {/* Search Box */}
              <div className="relative w-full md:max-w-md">
                ...
                <input
                  ...
                  className="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-200/80 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 focus:bg-white transition-all text-slate-800 placeholder-slate-400 font-medium"
                  autoFocus
                />
              </div>
              ...
              <div className="flex items-center gap-2 overflow-x-auto w-full py-1 scrollbar-none">
                <div className="text-slate-400 p-1.5"><SlidersHorizontal size={16} /></div>
            ...
            <div className="w-full lg:w-auto animate-in fade-in duration-200">
              <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                <ChefHat className="text-primary w-6 h-6" /> Explore Restaurants
              </h1>
              <p className="text-sm text-slate-400 font-medium relative z-10">Pre-book tables, order food, and bypass wait queues instantly.</p>
            </div>
          )}
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto shrink-0">
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 p-2 sm:px-4 sm:py-2.5 rounded-xl text-sm font-bold transition-all duration-200 border ${
                showSearch 
                  ? 'bg-slate-100 text-slate-700 border-slate-200 hover:bg-slate-200' 
                  : 'bg-white text-slate-600 border-slate-200/60 hover:text-slate-900 hover:bg-slate-50'
              }`}
            >
              {showSearch ? (
                <><X size={16} /> <span className="hidden sm:inline">Close</span></>
              ) : (
                <><Search size={16} /> <span className="hidden sm:inline">Search & Filter</span></>
              )}
            </button>
